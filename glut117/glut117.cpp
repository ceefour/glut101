...
cell material_Ke[4] = {
	...
	"Specifies red emitted light intensity of the material.",
	"%.2f" },
	...
	for (i = 0; i < TESS; i++) {
		...
		82 / 117
			for (j = 0; j <= TESS; j++) {
				...
				glVertex3f(-1 + (float)(i + 1) / TESS * 2, -1.0, -
					1 + (float)j / TESS * 2);
			}
		glEnd();
	}
...
void new_material(float* material)
{
...
void lighting_mouse(int x, int y)
{
...
void material_mouse(int button, int state, int x, int y)
{
	selection = 0;
	if (state == GLUT_DOWN) {
		lighting_mouse(x, y);
...
void spotlight_mouse(int button, int state, int x, int y)
{
	selection = 0;
	if (state == GLUT_DOWN) {
		lighting_mouse(x, y);
...
void lmodel_mouse(int button, int state, int x, int y)
{
	selection = 0;
	if (state == GLUT_DOWN) {
		lighting_mouse(x, y);
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	glutAttachMenu(GLUT_RIGHT_BUTTON);
	command = glutCreateSubWindow(window, GAP + 256 + GAP, GAP + 256 + GAP,
		256, 256);
